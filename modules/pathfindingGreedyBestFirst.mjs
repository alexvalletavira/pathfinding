...
export function greedyFirstBest(nodos, nodosConserve, aniNode, mapaInvestigado, height, width, startX, startY, endX, endY, mapa) { //dijkstra pathfinding
        ...
        for (let i = -1; i <= 1; i++) {
            ...
            for (let j = -1; j <= 1; j++) {
                ...
                if ((i != 0 || j != 0) && (i == 0 || j == 0)) {
                    ...
                    found = addNodeGreedyFirstBest(nodos[check], i, j, height, width, mapa, nodos, nodosConserve, mapaInvestigado, aniNode, startX, startY, endX, endY);
...
function addNodeGreedyFirstBest(nodo, direccionX, direccionY, height, width, mapa, nodos, nodosConserve, mapaInvestigado, aniNode, startX, startY, endX, endY) {
    ...
    if ((nodo.posX + direccionX) >= 0 && (nodo.posX + direccionX) < height && (nodo.posY + direccionY) >= 0 && (nodo.posY + direccionY) < width) {
        ...
        if (mapa[nodo.posX + direccionX][nodo.posY + direccionY] != '#' && mapaInvestigado[nodo.posX + direccionX][nodo.posY + direccionY] != 'i') {
            ...
            let nodo2 = {
                ...
                valor: distancia(nodo.posX + direccionX, endX, nodo.posY + direccionY, endY)
            };
