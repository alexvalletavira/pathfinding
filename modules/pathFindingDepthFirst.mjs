...
export function depthFirst(nodos, nodosConserve, aniNode, mapaInvestigado, height, width, startX, startY, mapa) { //depthfirst pathfinding
        ...
        for (let i = -1; i <= 1; i++) {
            ...
            for (let j = -1; j <= 1; j++) {
                ...
                if ((i != 0 || j != 0) && (i == 0 || j == 0)) {
                    ...
                    found = addNodeDepth(nodos[0], i, j, height, width, mapa, nodos, nodosConserve, mapaInvestigado, aniNode);
...
function addNodeDepth(nodo, direccionX, direccionY, height, width, mapa, nodos, nodosConserve, mapaInvestigado, aniNode) {
    ...
    if ((nodo.posX + direccionX) >= 0 && (nodo.posX + direccionX) < height && (nodo.posY + direccionY) >= 0 && (nodo.posY + direccionY) < width) {
        ...
        if (mapa[nodo.posX + direccionX][nodo.posY + direccionY] != '#' && mapaInvestigado[nodo.posX + direccionX][nodo.posY + direccionY] != 'i') {
            ...
            return mapa[nodo.posX + direccionX][nodo.posY + direccionY] === 'e'; //retornamos si es final y si se puede colocar.
